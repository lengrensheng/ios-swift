...
public class VideoSplashViewController:UIViewController{
    ...
    public var alawaysRepeat:Bool = true {
        didSet{
            if alawaysRepeat{
                NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(self.playerItemDidReachEnd), name:AVPlayerItemDidPlayToEndTimeNotification, object: moviePlayer.player?.currentItem)
            }
        }
    }
    ...
    private func setMoivePlayer(url:NSURL){
    ...
    func compressVideo(url:NSURL){
        let videoCutter = VideoCutter()
        videoCutter.copyVideoWithUrl(videoUrl: url, startTime: startTime, duration:duration){
            (videoPath,error) -> Void in
